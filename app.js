...
// 9. محرك التأثيرات ثلاثية الأبعاد (3D Card Tilt Effect)
function init3DTiltEffect() {
  ...
  document.querySelectorAll(".product-card, .feature-card, .info-card, .payment-card").forEach(applyTilt);

  // مراقبة الكروت المضافة ديناميكياً
  const observer = new MutationObserver(() => {
    document.querySelectorAll(".product-card, .feature-card, .payment-card").forEach(applyTilt);
  });
